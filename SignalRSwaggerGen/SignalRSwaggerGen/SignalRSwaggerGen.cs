...
namespace SignalRSwaggerGen
{
    ...
    public sealed class SignalRSwaggerGen : IDocumentFilter
    {
                    ...
                    {
                        {
                            ...
                            new OpenApiOperation
                            {
                                ...
                                RequestBody =  ToOpenApiRequestBody(context, method)
                            }
                        }
                    }
                });
        ...
        private static OpenApiRequestBody ToOpenApiRequestBody(DocumentFilterContext context, MethodInfo method) {


            if (method.GetCustomAttribute<SignalRHiddenAttribute>() != null) return null;
            

            var requestBodyAttribute  = method.GetCustomAttribute<SignalRRequestBodyAttribute>() ;
             

            if (requestBodyAttribute is null || GetOpenAPiMediaTypeContent(
                context,
                requestBodyAttribute.RequestType,
                out var mediaTypeContent)) {
                return null;
            }
        ...
        private static OpenApiResponses ToOpenApiResponses(DocumentFilterContext context, ParameterInfo returnArg)
        {
            ...
            foreach (var returnAttribute in returnAttributes)
            {
                ...
                if (GetOpenAPiMediaTypeContent(context, type, out var mediaTypeContent))
                    continue;
        ...
        private static bool GetOpenAPiMediaTypeContent(
            DocumentFilterContext context,
            Type type,
            out Dictionary<string, OpenApiMediaType> mediaTypeContent) {
            if (!TryGetReturnType(type, out type)) {
                mediaTypeContent = null;
                return true;
            }
            ...
            var mediaType = new OpenApiMediaType {
                Schema = schema.Reference == null
                    ...
                    : new OpenApiSchema {
                        Reference = new OpenApiReference {
                            Id = schema.Reference.Id, Type = ReferenceType.Schema
                        }
                    }
            };
            mediaTypeContent = new Dictionary<string, OpenApiMediaType> {
